  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-24 bg-[#111]/90 backdrop-blur-2xl border-t border-white/5 px-8 flex items-center justify-between z-50 noise group/player">
      ...
      <div className="flex items-center gap-4 w-1/3">
        <div className="w-14 h-14 bg-white/5 border border-white/10 rounded-xl relative overflow-hidden shadow-lg shrink-0">
          {currentTrack.coverUrl ? (
            <img src={currentTrack.coverUrl} alt={currentTrack.name} className="w-full h-full object-cover" />
          ) : (
            <div className="absolute inset-0 bg-gradient-to-br from-[#e8351e]/20 to-transparent" />
          )}
        </div>
      ...
      <div className="flex flex-col items-center gap-2 w-1/3">
        ...
        <div className="flex items-center gap-3 w-full max-w-lg">
          ...
          <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden relative group cursor-pointer" onClick={handleSeek}>
            <div
      ...
      <div className="flex items-center gap-6 w-1/3 justify-end">
        <div className="flex items-center gap-3">
          ...
          <div className="w-24 h-1.5 bg-white/5 rounded-full overflow-hidden relative group cursor-pointer" onClick={handleVolumeChange}>
            <div className="h-full bg-white/60 group-hover:bg-white transition-colors" style={{ width: `${volume * 100}%` }} />
          </div>
        </div>
      </div>
    </div>
